...
26 个 Issue（已完成 9 个：p1-001~009），258 个后端测试通过（Jest）。

| 阶段 | 内容 | 状态 |
|------|------|------|
| P1.0 | API key 轮换 + CORS 中间件 + CI 搭建 | ✅ 完成（p1-001, p1-002） |
| P1.1 | 规则引擎 v2 重构（特征提取器 + Prompt v2 锚点 + 双模型编排 + Guard + Degrade-Report + Filler 性能优化） | ✅ 7/8（p1-003~009），p1-010 前端适配 待开始 |
