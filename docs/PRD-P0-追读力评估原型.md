...
构建一个本地可运行的追读力评估原型。作者粘贴一章文本，系统通过规则引擎（本地计算）+ LLM 评估（DeepSeek API）并行分析，输出包含综合评分、爽点密度、Hook 强度、章末悬念、节奏曲线、注水检测和改进建议的完整报告。无用户系统、无部署，纯本地验证核心假设：**作者是否愿意为这类质量评估工具付费？**
...
13. 作为网文作者，我想在输入少于 100 字时收到友好提示，而不是硬性报错
...
- Zod schema 定义 LLM 返回结构：hookScore (0-10), cliffhangerScore (0-10), consistencyIssues (array), highlights (array), suggestions (array)
- API Key 从环境变量 `DEEPSEEK_API_KEY` 读取
- 模块接口：`evaluateWithLLM(text: string, prompt: string): Promise<LLMResult>`

### 模块 2: Rule Engine

三个独立分析器，均可纯本地运行，无外部依赖：
...
- 动作:对话:描写比例评分
- 输出：0-10 分 + 节奏曲线数据 `[{paragraph, tension, type}]`
...
- 编排器：使用 `Promise.all` 并行执行 Rule Engine（3 个分析器）+ LLM（3 个评估任务）
- 合并策略：Rule Engine 结果始终成功；LLM 结果任一失败时标记 `isPartial: true`
- 综合评分公式：`Hook × 0.3 + Climax × 0.3 + Cliffhanger × 0.25 + Pacing × 0.15`，保留 1 位小数
- 部分评估：仅返回 Rule Engine 结果 + 黄色提示标记
...
请求体：
- `chapterText`: string（必填，100-50000 字符）
...
| Evaluation Pipeline | 集成测试 | 完整评估流程（mock LLM → 验证合并结果）、部分失败处理（LLM 超时 → isPartial=true）、综合评分计算正确性 |
...
| 追读力评估管线 | 5 天 | Rule Engine + LLM 并行执行、结果合并 |
