...
1. **`detectDivergence(scoresA, scoresB)`** — 比较 DeepSeek 和 Claude 的 4 维度分数，任一维度差异 > 2 分时标记为 divergence
...
- [ ] `detectDivergence(deepseekScores, claudeScores)` 返回 `DivergenceReport | null`
- [ ] 任一维度差值 > 2 时标记为分歧（如 hook: 6 vs 3.5, delta=2.5 → 标记）
- [ ] 差值恰好 2.0 时不标记（边界值：hook: 6 vs 4, delta=2.0 → 不标记）
- [ ] 分歧报告包含：维度名、DeepSeek 分数、Claude 分数、delta
