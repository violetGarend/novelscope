...
4. **评分校准 + 多模型对比** — 修复趋中评分，增加 DeepSeek vs 豆包 双模型对比
...
21. 作为网文作者，我希望看到 DeepSeek 和豆包两个模型的评分对比，以便获得更可靠的评估
...
- Prompt v2 核心变更：
  ...
  - 与参考文本对比而非绝对评分
- 多模型编排：
  - DeepSeek（主）+ 豆包（辅），并行调用
  - 豆包 API key 可选：未配置时仅用 DeepSeek
...
| LLM Multi-Provider | 单元测试 | 双模型并行调用、豆包 fallback、mock API 错误处理、分歧检测逻辑 |
...
3. **双模型成本** — DeepSeek + 豆包 并行调用使每次评估成本翻倍，需监控并与配额策略协调
